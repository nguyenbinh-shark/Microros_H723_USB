...
typedef struct kf_t
{
    ...
    float *MeasurementDegree;     // The element value in H that corresponds to each measurement
    float *MatR_DiagonalElements; // Measurement noise variance for each measurement
    float *StateMinVariance;      // Minimum state variance to suppress filter excessive convergence
    uint8_t *temp;

    // User-defined flags, used as flags to determine whether to skip certain Kalman filter equations
