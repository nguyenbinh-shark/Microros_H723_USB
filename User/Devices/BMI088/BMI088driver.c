...
uint8_t BMI088_init(SPI_HandleTypeDef *bmi088_SPI, uint8_t calibrate)
{
    ...
    if (calibrate)
            { // Calibration requested: keep the IMU level and completely still.
...
// Calibrate gyro and accelerometer zero bias.
void Calibrate_MPU_Offset(IMU_Data_t *bmi088)
{
    static float startTime;
    static uint16_t CaliTimes = 6000; // Use enough samples for a reliable gyro bias estimate.
	  static uint16_t acc_CaliTimes = 6000; // Use enough samples for a reliable accel bias estimate.
    ...
    do
    {
        ...
        for (uint16_t i = 0; i < CaliTimes; i++)
        {
            ...
            if (buf[0] == BMI088_GYRO_CHIP_ID_VALUE)
            {
                ...
                bmi088->GyroOffset[0] += bmi088->Gyro[0]; // Accumulate gyro bias samples.
                bmi088_raw_temp = (int16_t)((buf[5]) << 8) | buf[4];
            ...
            // If spread is too large, external disturbance is assumed; restart calibration.
        ...
        // Average samples to get calibration results.
        bmi088->gNorm /= (float)CaliTimes;
        for (uint8_t i = 0; i < 3; i++)
            bmi088->GyroOffset[i] /= (float)CaliTimes;

        // Record IMU temperature at calibration time.
        ...
        // Calibrate accelerometer scale factor from calibration result.
    bmi088->AccelScale = 9.81f / bmi088->gNorm;
		 ...
		 for (uint16_t i = 0; i < acc_CaliTimes; i++)
     {
      ...
      bmi088->Accel[0] = bmi088_raw_temp * BMI088_ACCEL_SEN * bmi088->AccelScale;
			bmi088->AccelOffset[0]=bmi088->AccelOffset[0]*0.8f+bmi088->Accel[0]*0.2f; // First-order low-pass filter for accel bias.
			 
      bmi088_raw_temp = (int16_t)((buf[3]) << 8) | buf[2];
      bmi088->Accel[1] = bmi088_raw_temp * BMI088_ACCEL_SEN * bmi088->AccelScale;
			bmi088->AccelOffset[1]=bmi088->AccelOffset[1]*0.8f+bmi088->Accel[1]*0.2f; // First-order low-pass filter for accel bias.
			 
      bmi088_raw_temp = (int16_t)((buf[5]) << 8) | buf[4];
      bmi088->Accel[2] = bmi088_raw_temp * BMI088_ACCEL_SEN * bmi088->AccelScale;
			bmi088->AccelOffset[2]=bmi088->AccelOffset[2]*0.8f+bmi088->Accel[2]*0.2f; // First-order low-pass filter for accel bias.
...
void BMI088_Read(IMU_Data_t *bmi088)
{
		...
		if (caliOffset)
    {
			...
			bmi088_raw_temp = (int16_t)((buf[5]) << 8) | buf[4];
            bmi088->Accel[2] = bmi088_raw_temp * BMI088_ACCEL_SEN * bmi088->AccelScale; // Keep Z bias unchanged because it includes gravity.
		}
		else
		{
			bmi088_raw_temp = (int16_t)((buf[1]) << 8) | buf[0];
